...
async def execute_python_code(sandbox_id: str, code: str, timeout: Optional[int] = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        safe_code = quote(code)
        
        # Execute the code in the specified sandbox with enhanced security
        result = sandbox_manager.execute_code_secure(sandbox_id, safe_code, timeout)
        ...
        # This allows for structured data return from Python code
        import json
        try:
            if stdout and (stdout.startswith("{") or stdout.startswith("[")):
                stdout = json.loads(stdout)
        except json.JSONDecodeError:
            # If JSON parsing fails, keep the original string output
            pass
        
        # Return the execution results
        return {
            "success": result["exit_code"] == 0,  # Success if exit code is 0
            "output": stdout,
            "error": stderr if stderr else None,  # Only include error if there is one
...
"""
        ...
        result = sandbox_manager.execute_code_secure(sandbox_id, code, timeout)
        
        # Process the output from the installation
        stdout = result["stdout"].strip()
        stderr = result["stderr"].strip()
        
        # Try to parse stdout as JSON for structured response
        import json
        try:
            if stdout:
                stdout = json.loads(stdout)
